...
class check_package_status
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if(\DB::table('administration_users')->where('created_by_code',auth()->user()->id)->where('user_code',auth()->user()->id))
        {
            ...
            if ($creation_time->username != 'test')
            {
    ...
    else if(\DB::table('administration_users')->where('created_by_code','<>',auth()->user()->id)->where('user_code',auth()->user()->id))
    {
        $creation_time=\DB::table('users')->where('id',auth()->user()->id)->first();
         $created = new Carbon($creation_time->time_stamp_for_record_creation);
         $now = Carbon::now();
